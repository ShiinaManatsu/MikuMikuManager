...
namespace MMD
{
	public class PMDConverter
	{
		...
		void SetDrive(PMDFormat.Joint joint, ConfigurableJoint conf)
		{
			...
			{
				drive = new JointDrive {  positionSpring = joint.spring_rot.x };
                conf.angularXDrive = drive;
			}

            if (joint.spring_rot.y == 0f && joint.spring_rot.z == 0f) return;
            drive = new JointDrive {positionSpring = (joint.spring_rot.y + joint.spring_rot.z) * 0.5f};
